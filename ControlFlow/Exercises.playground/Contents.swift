...
for movieTitle in movieTitleArray {
    print("\(movieTitle) with Puppets!")
}
//: ### Exercise 2
...
for mountainViewRent in oldMountainViewRent {
    newMountainViewRent.append(mountainViewRent * 1.2)
}
...
for (area, rapperPhoneNumber) in rapperPhoneNumbers {
    if rapperPhoneNumber.range(of: "(415)") != nil {
        rapperPhoneNumbers[area] = rapperPhoneNumbers[area]?.replacingOccurrences(of: "(415)", with: "(510)")
    }
}
